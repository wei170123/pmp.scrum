angular.module('ptLineBot_SportsDB').controller('UpperLimbsController', ['$rootScope', '$scope', '$state', '$stateParams', '$window', 'UpperLimbsService', 'AlertService', function ($rootScope, $scope, $state, $stateParams, $window, UpperLimbsService, AlertService) {
    self = this;
    self.checkedList = [];

    var init = function () {
        $rootScope.sports.uid = $stateParams.uid;
        UpperLimbsService.getSportsList(function (data) {
            self.sports = data;
        });
    };
    ...
    self.confirm = function () {
        if ($rootScope.sports.details.length) {
            var callBack = function () {
                UpperLimbsService.confirm($rootScope.sports, function (data) {
                    if (data.error)
