angular.module('ptLineBot_SportsDB').controller('NeckController', ['$rootScope', '$location', '$state', '$stateParams', '$window', 'NeckService', 'AlertService', function ($rootScope, $location, $state, $stateParams, $window, NeckService, AlertService) {
    self = this;
    self.checkedList = [];

    var init = function () {
        $rootScope.sports.uid = $stateParams.uid;
        NeckService.getSportsList(function (data) {
            self.sports = data;
        });
    };
    ...
    self.confirm = function () {
        if ($rootScope.sports.details.length) {
            var callBack = function () {
                NeckService.confirm($rootScope.sports, function (data) {
                    if (data.error)
