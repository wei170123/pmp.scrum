angular.module('ptLineBot_SportsDB').controller('LowerLimbsController', ['$rootScope', '$location', '$state', '$stateParams', '$window', 'LowerLimbsService', 'AlertService', function ($rootScope, $location, $state, $stateParams, $window, LowerLimbsService, AlertService) {
    self = this;
    self.checkedList = [];

    var init = function () {
        $rootScope.sports.uid = $stateParams.uid;
        LowerLimbsService.getSportsList(function (data) {
            self.sports = data;
        });
    };
    ...
    self.confirm = function () {
        if ($rootScope.sports.details.length) {
            var callBack = function () {
                LowerLimbsService.confirm($rootScope.sports, function (data) {
                    if (data.error)
